...
sidebar_position: 1
description:
  Master TypeScript generics to write reusable, type-safe code with generic
  functions, classes, interfaces, and constraints.
keywords:
  [
    typescript generics,
    generic types,
...
while maintaining type safety. They're one of TypeScript's most powerful
features.
...
Make type parameters constant to preserve literal types in generic functions.
...
### Array and Tuple Preservation

```typescript
// Without const - loses tuple structure
function getArray<T extends readonly unknown[]>(arr: T): T {
  return arr;
}

const arr = getArray([1, 2, 3] as const);
// Type: readonly [1, 2, 3] (works, but requires 'as const')

// With const type parameter - preserves tuples automatically
function getArray<const T extends readonly unknown[]>(arr: T): T {
  return arr;
}

const arr = getArray([1, 2, 3]);
// Type: readonly [1, 2, 3] (preserves tuple without 'as const'!)
```

### Object Preservation

```typescript
function getConfig<const T extends Record<string, unknown>>(config: T): T {
  return config;
}
...
// Type: { theme: "dark"; fontSize: 14 }
// Literal types preserved!
```

**Benefits:**

- ✅ Preserves literal types automatically
- ✅ Better type inference for tuples
- ✅ More precise return types
- ✅ Less need for `as const` assertions
