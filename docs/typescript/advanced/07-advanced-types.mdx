...
sidebar_position: 7
description:
  Master advanced TypeScript types including mapped types, conditional types,
...
Master **advanced type patterns** in TypeScript including mapped types,
conditional types, template literals, and complex type transformations.
...
Ensures a value matches a type without changing the inferred type.

```typescript
// Problem: Type assertion loses literal types
const colors = {
  primary: '#ff0000',
  secondary: '#00ff00',
} as const;
// Type is too narrow: { readonly primary: "#ff0000"; readonly secondary: "#00ff00" }

// Solution: satisfies preserves types while checking
const colors = {
  primary: '#ff0000',
  secondary: '#00ff00',
} satisfies Record<string, string>;
// Type: { primary: "#ff0000"; secondary: "#00ff00" }
// Still has literal types, but satisfies Record<string, string>
...
// TypeScript checks shape, preserves literal types
function getApiUrl() {
  return config.apiUrl; // Type: "https://api.example.com" (not just string)
}
```

### `const` Type Parameters (TypeScript 5.0+)

Make type parameters constant to preserve literal types.
...
const result = getValue('hello');
// Type: "hello" (preserves literal!)

// Array example
function getArray<const T extends readonly unknown[]>(arr: T): T {
  return arr;
}

const arr = getArray([1, 2, 3] as const);
// Type: readonly [1, 2, 3] (preserves tuple!)
```

**Use Cases:**

- Preserve literal types in generic functions
- Better type inference for tuples
- More precise return types

**Browser Support**: TypeScript 5.0+
...
- [Type Guards](../intermediate/type-guards)
