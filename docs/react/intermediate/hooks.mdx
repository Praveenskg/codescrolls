...
**Note**: `useFormStatus` must be used inside a `<form>` element with an action.
...
Provides a stable event callback that always accesses the latest props and state without causing re-renders.
...
- ✅ Perfect for WebSocket handlers, analytics
