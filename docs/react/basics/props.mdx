...
sidebar_position: 2
description:
  Learn how to pass data between React components using props, including prop
  types, default props, and best practices.
keywords:
  [
    react props,
    component props,
...
**Props** (short for "properties") are how you pass data from parent components
to child components in React. They are **read-only** and help make components
reusable.
...
Props are arguments passed into React components, similar to function
parameters. They allow you to pass data and event handlers down the component
tree.
...
function App() {
  return <UserCard name="Praveen Singh" age={26} role="Full Stack Developer" />;
}
```
...
The special `children` prop allows you to pass components or elements between
...
Component.propTypes = {
  ...
  // One of many types
  multi: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
};
```
...
**Props are the foundation of React's data flow. Master them to build flexible,
reusable components!** 🚀
