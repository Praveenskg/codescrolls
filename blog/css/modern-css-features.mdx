...
description:
  'Master modern CSS features including Container Queries, :has() selector, CSS
  Nesting, Subgrid, Cascade Layers, View Transitions API, and modern color
  functions. Learn when and how to use these powerful features.'
...
CSS has evolved dramatically in recent years, introducing powerful features that
make styling more intuitive, maintainable, and performant. This comprehensive
guide covers the latest CSS features that are revolutionizing how we build
modern web interfaces, from Container Queries to the `:has()` selector and
beyond.
...
Container Queries are one of the most revolutionary CSS features, allowing you
to style elements based on their container's size rather than the viewport. This
enables true component-based responsive design.

### The Problem with Media Queries

```css
/* ❌ Problem: Media queries are viewport-based */
...
The `:has()` pseudo-class (also called the "parent selector") allows you to
select elements based on their descendants. This was a long-requested feature
that enables powerful styling patterns.
...
Native CSS nesting allows you to write cleaner, more maintainable stylesheets
without preprocessors. It's now supported natively in modern browsers!
...
Subgrid allows grid items to participate in their parent grid's layout, enabling
more flexible and powerful grid designs.

### The Problem

```css
/* ❌ Without subgrid: Grid items can't align with parent */
.grid {
  display: grid;
...
reducing specificity conflicts and making stylesheets more maintainable.
...
preventing style leakage and enabling better component encapsulation.
...
**Browser Support**: Chrome 118+, Firefox (in development), Safari (in
development)

---

## 🎨 Modern Color Functions {#modern-color-functions}

Modern CSS introduces powerful new color functions that provide better color
manipulation and more perceptually uniform color spaces.
...
.button {
  ...
  /* Lightness 70%, Chroma 0.15, Hue 250° */
}
...
The View Transitions API enables smooth transitions between page states,
creating native-like app experiences.
...
**Browser Support**: Chrome 111+, Firefox (in development), Safari (in
development)
...
Modern CSS introduces new typography features for better text layout and
control.
...
- `text-box-trim`: Chrome 118+, Firefox (in development), Safari (in
  development)

---

## 🔧 The @property Rule {#the-property-rule}

The `@property` rule allows you to define custom CSS properties with types,
enabling better animations and transitions.
...
```css
/* ✅ Good: Component-based responsive design */
.card-container {
  container-type: inline-size;
}
...
/* ❌ Avoid: Viewport-based for components */
@media (min-width: 768px) {
  .card {
...
```css
/* ✅ Good: Perceptually uniform colors */
.button {
...
| ----------------- | ------ | ------- | ------ | -------------- |
| Container Queries | 105+   | 110+    | 16.0+  | ✅ Stable      |
| `:has()` Selector | 105+   | 121+    | 15.4+  | ✅ Stable      |
| CSS Nesting       | 112+   | 117+    | 16.5+  | ✅ Stable      |
| Subgrid           | 117+   | 71+     | 16.0+  | ✅ Stable      |
| Cascade Layers    | 99+    | 97+     | 15.4+  | ✅ Stable      |
| `@scope`          | 118+   | 🔄 Dev  | 🔄 Dev | 🚧 In Progress |
| `color-mix()`     | 111+   | 113+    | 16.2+  | ✅ Stable      |
| `oklch()`         | 111+   | 113+    | 15.4+  | ✅ Stable      |
| View Transitions  | 111+   | 🔄 Dev  | 🔄 Dev | 🚧 In Progress |
| `text-wrap`       | 114+   | 🔄 Dev  | 🔄 Dev | 🚧 In Progress |
| `@property`       | 85+    | 72+     | 16.4+  | ✅ Stable      |

**Legend**: ✅ Stable | 🚧 In Progress | 🔄 Dev (Development)
...
These features make CSS more powerful and your code more maintainable. Start
incorporating them into your projects today!
